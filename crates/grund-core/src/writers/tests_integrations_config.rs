...
fn agent_partial_reports_unknown_agents_and_keys() {
    ...
    assert!(
        scan.problems[0].1.contains("unknown agent `codx`")
            && scan.problems[0]
                .1
                .contains("codex, claude, gemini, copilot, zed, pi"),
        "{:?}",
        scan.problems
    );
...
fn agent_override_installs_into_its_own_table() {
    ...
    assert!(
        written.starts_with(base),
        "base must be untouched: {written}"
    );
...
fn conversation_preference_appends_and_updates() {
    ...
    let (plain, outcome) = install_conversation_preference(existing, ConversationRendering::Plain);
    assert_eq!(outcome, BlockOutcome::Appended);
    assert_eq!(
        plain,
        "keep = true\n\n[reference]\nconversation = \"plain\"\n"
    ...
    let (linked, outcome) = install_conversation_preference(&plain, ConversationRendering::Link);
    assert_eq!(outcome, BlockOutcome::Updated);
    assert!(linked.starts_with("keep = true\n\n[reference]\n"));
    assert!(linked.ends_with("conversation = \"link\"\n"));
    let (again, outcome) = install_conversation_preference(&linked, ConversationRendering::Link);
    assert_eq!(outcome, BlockOutcome::Unchanged);
...
fn conversation_preference_reads_equivalent_toml_spellings() {
    for text in [
        ...
        let (same, outcome) = install_conversation_preference(text, ConversationRendering::Link);
        assert_eq!(outcome, BlockOutcome::Unchanged);
        assert_eq!(same, text);
    }
}
...
fn user_config_reports_bad_values_and_duplicates_without_failing() {
    ...
    assert_eq!(
        ...
        vec![(
            3,
            "ignoring duplicate `reference.conversation`; the first one is used".to_string()
        )]
    );
...
fn agent_guidance_block_tracks_user_preference() {
    ...
    let (linked, outcome) = install_agent_guidance_block(
        &with_tail,
        ConversationRendering::Link,
        ConversationTarget::Path,
    )
    ...
    for text in [
        ...
        assert!(
            !text.contains('\u{a7}'),
            "global block must name no marker: {text}"
        );
    }

    let (again, outcome) = install_agent_guidance_block(
        &linked,
        ConversationRendering::Link,
        ConversationTarget::Path,
    )
...
fn vscode_installation_state_checks_owned_files() {
    ...
    write(
        &root.join(".grund-version"),
        &INTEGRATIONS_BLOCK_VERSION.to_string(),
    );
