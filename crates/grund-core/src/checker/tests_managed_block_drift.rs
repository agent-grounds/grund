...
fn clickable_citations_drift_is_reported() {
    ...
    assert!(
        report
            .errors
            .iter()
            .any(|d| d.code == "agents-init" && d.message.contains("clickable citations differ")),
...
fn agent_guidance_block_upgrades_older_version_in_place() {
    ...
    let (updated, outcome) = install_agent_guidance_block(
        existing,
    ...
    assert!(
        updated.contains("In repositories with a `grund.toml` (at the root or under `.agents/`):")
    );
    // §DF-repo-conversation-opinion.2.3: the machine-local `plain` wins over
    // a repository's linked-citations opinion.
    assert!(
        updated.contains("Follow this even when repository instructions ask for linked citations")
    );
