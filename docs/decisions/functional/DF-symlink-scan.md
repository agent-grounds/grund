...
An external directory link contributes no files to `fmt`, because [§DF-symlink-scan.2.2](DF-symlink-scan.md#22-the-canonical-project-root-fences-directory-links-not-file-links-or-scan-paths) prunes
...
A link whose target is inside the root is written through, which leaves one residue that is accepted rather than fixed. The file is read once, under the surviving spelling ([§DF-symlink-scan.2.3](DF-symlink-scan.md#23-the-report-names-the-link-never-the-target)), so `--cross-refs` anchors its relative links to that spelling: the link is right where grund read the file and wrong at its other name. There is no anchor that is right at both — a relative path resolves against the directory the reader opened, and the file has two — so the only real fix is one name per file. Naming it beats a rule that pretends to solve it.
...
- External file links and plain parent-relative includes can still read bytes outside the project root; external directory links cannot ([§DF-symlink-scan.2.2](DF-symlink-scan.md#22-the-canonical-project-root-fences-directory-links-not-file-links-or-scan-paths)).
...
- `grund fmt --write` reads an external file link and does not write it, naming it on stderr instead ([§DF-symlink-scan.2.5](DF-symlink-scan.md#25-fmt---write-refuses-an-external-file-link-external-directory-links-never-reach-it)). Files below an external directory link never reach it; inside the root it writes through, with the cross-reference anchoring residue [§DF-symlink-scan.2.5](DF-symlink-scan.md#25-fmt---write-refuses-an-external-file-link-external-directory-links-never-reach-it) records.
...
| Option | Why rejected |
|---|---|
| Report the skipped link and keep not reading it | Closes the silent half of the bug and leaves the false-alarm half: the linked file's citations stay unread, so what it cites is still reported unused ([§DF-symlink-scan.2.1](DF-symlink-scan.md#21-follow-the-link-do-not-report-and-skip-it)). |
...
| Make member-local runs discover and load an ancestor workspace | Imports workspace expansion and its full error surface into every independent run to answer a boundary question the project's own canonical root already answers ([§DF-symlink-scan.2.2](DF-symlink-scan.md#22-the-canonical-project-root-fences-directory-links-not-file-links-or-scan-paths)). |
| Refuse external file links too | Re-creates the original false alarm for a citation plainly present in a linked file and breaks common in-tree entrypoint aliases ([§DF-symlink-scan.2.1](DF-symlink-scan.md#21-follow-the-link-do-not-report-and-skip-it)). |
| Report findings at the resolved target path | Unjumpable, unrenderable under `relative_paths`, and it breaks the `--full` additivity rule, which requires the in-scope lines to be identical spelling included ([§DF-symlink-scan.2.3](DF-symlink-scan.md#23-the-report-names-the-link-never-the-target)). |
| Canonicalize every walked file so aliases always collapse | A `realpath` per file on every run of every repository, to serve the trees that have a symlink in them. The walk is where [§GOAL-fast-feedback](../../goals.md#goal-fast-feedback-grund-must-be-as-fast-as-possible) is set ([§DF-symlink-scan.3](DF-symlink-scan.md#3-consequences)). |
| Arm the identity pass with a flag the first symlink raises | The same `realpath` per file as the row above, bought by one link anywhere in the tree — and real repositories have one, `CLAUDE.md -> AGENTS.md` among them. Measured at ~2.7x on a 20 000-file tree with a single link. What the walk records instead is the *list* of files that can wear a second name, so the cost tracks the links rather than the repository ([§DF-symlink-scan.3](DF-symlink-scan.md#3-consequences)).
| Abort the scan on the first unresolvable link | One dangling link takes the whole report with it, which is the opposite of what [§FS-check.2](../../functional-spec/FS-check.md#2-outputs) promises about a file the walk cannot read ([§DF-symlink-scan.2.4](DF-symlink-scan.md#24-a-link-the-walk-cannot-resolve-is-reported-and-the-walk-continues)). |
