...
- A workspace whose `[workspace]` block cannot be expanded now fails `cover`. Reading the members is what expanding them requires, and `cover` did not read them, so a duplicate alias, an invalid one, a missing member, or a nested block with nothing in scope all left it answering `0` over the root project while `grund list` on the same tree exited `2`. The same silence as the rest of this decision, one layer earlier: the run could not know what the tree contained and said nothing about it.
- Two further verdict changes, both from `0` to `2`, both in workspaces: a member's unreadable file now fails the run at the root, and so does the root's when a member was the reason the root was loaded. In both cases exit `2` is the code `check`, `list`, and `refs` already return for the same tree, so the change makes two installs of `grund` agree rather than making them differ ([§FS-non-goals.13](../../functional-spec/FS-non-goals.md#13-anything-that-would-let-two-grund-installs-disagree)).

  **Which clause licenses the move.** [§REQ-backwards-compatibility.5](../../requirements/REQ-backwards-compatibility.md#5-correcting-a-verdict-another-requirement-forbids) does, because the old `0` violated the separately declared prohibition in [§REQ-no-missed-citation.1](../../requirements/REQ-no-missed-citation.md#1-no-silent-skips): the run had not scanned the projects whose records were absent, yet claimed success. That requirement already applied when the verdict shipped; this decision records the conflict rather than inventing a new prohibition for the correction.

  Neither ordinary route fits. The [§REQ-backwards-compatibility.2](../../requirements/REQ-backwards-compatibility.md#2-the-deprecation-path) deprecation path cannot apply because there is no old form to carry beside a new one, and the [§REQ-backwards-compatibility.3](../../requirements/REQ-backwards-compatibility.md#3-loud-mechanical-migrations) mechanical-migration route has no command the tool ships that completes the change. Nor does [§REQ-backwards-compatibility.4](../../requirements/REQ-backwards-compatibility.md#4-what-was-never-a-promise) exempt it: the old run produced output, and exit `0` had a defined meaning under [§FS-cover.4](../../functional-spec/FS-cover.md#4-exit-codes). The answer was false, not undefined.

  The correcting release names the `0` to `2` verdict change, and every finding that replaces the old `0` names its location and an action the maintainer can take: `duplicate workspace project alias \`api\`` names the two `members` entries that collide, and a broken symlink names the path that cannot be read. This is therefore the bounded correction route in [§REQ-backwards-compatibility.5](../../requirements/REQ-backwards-compatibility.md#5-correcting-a-verdict-another-requirement-forbids), not a licence for ordinary policy tightening, feature removal, or a newly invented prohibition. The move is the opposite of the silence the requirement guards against ([§GOAL-no-silent-breakage](../../goals.md#goal-no-silent-breakage-changes-ship-through-a-deprecation-path)).
- A single-project tree with no qualified citations is byte-identical in both formats. One that *does* carry a qualified citation gains, in both formats, the rows it was dropping — the change is the same one, and naming it for text alone would read as though JSON escaped it.
- No **field** is added to a single-project tree's JSON: `project` appears only in workspace mode ([§DF-cover-workspace-scope.2.3](DF-cover-workspace-scope.md#23-the-project-field-is-added-and-only-in-workspace-mode)), so a consumer's parser sees the schema it was written against.
- [§AR-resolver.3](../../architecture/AR-resolver.md#3-downstream-commands-compose-not-duplicate) loses its `cover` carve-out: every query command now composes through `load_workspace_context`, and no consumer filters on `namespace.is_none()`.
