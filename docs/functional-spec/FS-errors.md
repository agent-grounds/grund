...
Emitted on **stdout** — it is the command's output (§1.1). Which form each command uses is §2.1.1.
...
- On **stderr** (§1.2) — it is not the command's output.
...
Where the message text still carries a location is §2.2.1; the exit code each prefix accompanies is §2.2.2; who uses the shape is §2.2.3; how it renders under `--format=json` is §2.2.4.
...
A *launch-time* `error:` (bad flag, unreadable config, missing path) is printed as raw text and is never JSON-ified; a *mid-walk* per-file failure collected by `grund check` is one of the report's diagnostics and is rendered in `--format=json` like the others (§5.2.3), still on stderr because it is not a finding about the spec graph.
...
How ambiguity and hint lines read is §2.3.1; who uses the shape is §2.3.2.
...
- **One base for every path in the line** — §3.1.

§3.2 to §3.6 pin particular messages: the unknown-project recovery shape and its wording migration, `check`'s channel marker, a missing fetch-backed declaration, and `agents-init`.
...
severity, and the default exit verdict do not change. The final `0.14.0` templates are §3.6.1.
...
- Non-deterministic ordering — the order each report keeps is §4.1.
...
How `check --only` and `--ignore` keep the contract is §4.2.
...
`grund check --only` and `--ignore` preserve §4's contract: selection precedes
...
Which ambiguity refusals carry `sites` is §5.2.1; the messages that stay raw text under any `--format` are §5.2.2; run-level diagnostics in `check`'s report are §5.2.3.
