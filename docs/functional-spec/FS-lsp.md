...
`textDocument/hover` answers on both sides of a citation. On a citation that resolves, it previews the body the CLI prints for it (§1.2.1); on a citation that carries a diagnostic, it returns nothing (§1.2.2). On a declaration-side title — a Markdown declaration heading, the same declaration written inline in a doc-comment, a numbered section heading (`<ID>.<section>`, §1.3.1), or an inline-spec stub title — it returns the title token and how much of the tree leans on it (§1.2.3 to §1.2.7).
...
The result's shape is §1.3.4; how values and named sections navigate is §1.3.5.
