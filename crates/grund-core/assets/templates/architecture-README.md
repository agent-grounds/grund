...
Internals — *how* this project is built. One file per spec; each H1 declares an `{AR_ID_SHAPE}` ID and the body is its contract. Citations from elsewhere in the tree (`§{AR_ID_SHAPE}.<section>`) resolve into these files.
