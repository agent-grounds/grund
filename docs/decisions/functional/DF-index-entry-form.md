...
**Checking the link target as well as its shape.** Rejected in [§DF-index-entry-form.2.2](DF-index-entry-form.md#22-full-link-is-the-link-fmt-would-write-here-not-has-an-anchor): it duplicates the anchor algorithm for no new coverage, and [§AR-ci](../../architecture/AR-ci.md#ar-ci-ci-mirrors-the-local-pre-commit-gate) already runs `grund fmt --write` and `lychee` in the same gate as `grund check --full`.

## 4. Consequences

The four hand-curated indexes in this repository were already conformant: 50 citations across them, 48 already links, and the two exceptions are the inline-code prose [§DF-index-entry-form.2.3](DF-index-entry-form.md#23-one-link-per-id-not-every-mention) exempts. The cost of the rule here was zero, which is the evidence that it is narrow enough.
