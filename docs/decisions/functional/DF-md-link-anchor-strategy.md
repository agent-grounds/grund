...
- [§FS-fmt.6.7](../../functional-spec/FS-fmt.md#67-configurability)'s `anchor_format` config gains the named-profile shape from [§DF-md-link-anchor-strategy.2.3](DF-md-link-anchor-strategy.md#23-renderer-profiles) above.
...
| **(c) Per-renderer format with no default.** User must pick a profile before `--cross-refs` works. | Not actually a separate option — it is how (a) gets shipped configurably. Without specifying the underlying slug strategy, this is a deferral, not a decision. Folded into (a) as the renderer-profile config in [§DF-md-link-anchor-strategy.2.3](DF-md-link-anchor-strategy.md#23-renderer-profiles). |
