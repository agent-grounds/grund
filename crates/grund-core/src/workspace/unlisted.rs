...
/// The release §FS-check.4.8.14's warning becomes an error in
...
/// §FS-check.4.8.15, §FS-lsp.1.1.3: the same findings for the five surfaces that have
...
fn block_warnings(
    ...
    // Quiet (§FS-check.4.8.2): this climb spells no alias path, so an ancestor it
    ...
    for dir in walked_dirs {
        ...
        // never meets as a file entry — it prunes hidden directories (§FS-check.4.8.1).
        ...
        // §FS-check.4.8.3: a project root of this run is never a candidate — without it
        ...
        // §FS-check.4.8.4 "one finding for one edit": one block the walk reached under
        ...
        match enclosing_workspace_of(dir, &config.cli_base, &mut ancestors) {
            ...
            // unexplained (§FS-check.4.8.2) rather than called unlisted.
            Err(_) => continue,
            Ok(None) => {}
        }
...
/// §FS-check.4.8.6: the sentence, built apart from the reporting so both channels
...
/// §FS-check.4.8.3: whether this canonical directory is one of the project roots the
