...
use super::scope_probe::effective_scope_reads_any_file_with;
use super::*;
use crate::config::Config;
use crate::testing::{test_root, write};
use crate::writers::{InitFsHome, InitNext};
...
fn effective_scope_rejects_files_the_scanner_skips() {
    ...
    write(
        &root.join("content/unsupported.txt"),
        "Not a configured extension.\n",
    );
    write(&root.join("content/.hidden.md"), "Hidden file.\n");
    write(
        &root.join("content/.private/note.md"),
        "Hidden directory.\n",
    );
    write(
        &root.join("content/excluded/note.md"),
        "Excluded directory.\n",
    );
    write(&root.join("content/.gitignore"), "ignored/\n");
    write(
        &root.join("content/ignored/note.md"),
        "Ignored directory.\n",
    );
    write(
        &root.join("content/templates/template.md"),
        "Unwalked kind home.\n",
    );
