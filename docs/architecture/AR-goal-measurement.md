...
| [§GOAL-token-economy](../goals.md#goal-token-economy-give-an-agent-the-right-amount-of-spec-not-the-whole-file) | Read/query specs and fixtures ([§FS-show](../functional-spec/FS-show.md#fs-show-grund-reads-a-single-declaration-body-by-id), [§FS-refs.3.3](../functional-spec/FS-refs.md#33---summary), [§FS-list.3.3](../functional-spec/FS-list.md#33---summary)); point-size measurements ([§FS-list.3.4](../functional-spec/FS-list.md#34---size--per-point-lead-and-full-body-measurements)) and the oversized-lead opt-in gate ([§FS-check.4.13](../functional-spec/FS-check.md#413-oversized-lead-opt-in)); default rationale in [§DF-show-default-token-cheap](../decisions/functional/DF-show-default-token-cheap.md#df-show-default-token-cheap-grund-show-defaults-to-the-cheap-read-the-full-body-is-opt-in). |
...
| [§GOAL-small-and-large](../goals.md#goal-small-and-large-start-small-configure-for-big) | Tiny conformant fixtures, configured large fixtures, and the 10k-file benchmark input ([§AR-benchmarks.1.1](AR-benchmarks.md#11-generated-fixtures-never-this-repository)). |

## 3. Requirement meters

| Requirement | Meter |
|---|---|
| [§REQ-backwards-compatibility](../requirements/REQ-backwards-compatibility.md#req-backwards-compatibility-an-upgrade-never-changes-a-verdict-quietly) | Block-version upgrade fixtures ([§FS-init.2.3.6](../functional-spec/FS-init.md#236-clickable-citations)), the frozen exit mapping ([§FS-cli.5](../functional-spec/FS-cli.md#5-exit-code-mapping-is-fixed)), config version gate ([§FS-config.5](../functional-spec/FS-config.md#5-schema-versioning)), release discipline ([§FS-distribution.4](../functional-spec/FS-distribution.md#4-release-process)), and `tests/integration/test_backwards_compatibility_routes.py`, which keeps the requirement's route enumeration exhaustive and requires every [§REQ-backwards-compatibility.5](../requirements/REQ-backwards-compatibility.md#5-correcting-a-verdict-another-requirement-forbids) correction to have an accepted decision, a separately declared prohibition, and a release record. Its central promise is not yet measured across releases ([§AR-goal-measurement.4](AR-goal-measurement.md#4-backwards-compatibility-is-not-metered-across-releases)). |
...
| [§REQ-no-data-loss](../requirements/REQ-no-data-loss.md#req-no-data-loss-grund-never-eats-user-content) | Idempotent re-run and `--force` fixtures ([§FS-init.3](../functional-spec/FS-init.md#3-non-intrusive-guarantees)), the never-rewritten list ([§FS-fmt.2.3](../functional-spec/FS-fmt.md#23-what-is-never-rewritten)), and install, idempotent re-run and in-place upgrade cases ([§FS-integrations.4](../functional-spec/FS-integrations.md#4-managed-writes---write)). |
...
| [§REQ-runs-offline](../requirements/REQ-runs-offline.md#req-runs-offline-verification-never-depends-on-an-external-service) | Offline query/check tests and the fetch-backed snapshot e2e and integration fixtures ([§FS-fetch](../functional-spec/FS-fetch.md#fs-fetch-grund-materializes-one-external-fact-snapshot)). |
...
| [§REQ-shipped-surfaces](../requirements/REQ-shipped-surfaces.md#req-shipped-surfaces-what-grund-ships-or-prints-resolves-where-it-lands) | The shipped-surface guard in `tests/integration/test_shipped_surfaces.py`, which matches this repository's declaration catalog against the embedded assets, their tree copies, the output goldens and the binaries' string literals; the asset-sync check that keeps tree and embedded copies one file ([§FS-init.5](../functional-spec/FS-init.md#5-agent-setup-instructions)); and the `scan = false` homes whose content ships verbatim ([§FS-config.3.4.7](../functional-spec/FS-config.md#347-scan--a-place-that-is-listed-not-walked)). |
...
## 4. Backwards compatibility is not metered across releases

The central promise of [§REQ-backwards-compatibility](../requirements/REQ-backwards-compatibility.md#req-backwards-compatibility-an-upgrade-never-changes-a-verdict-quietly) is not measured across releases: no gate runs a released and a candidate binary over the same repositories, so a checker change with current-version goldens updated can pass without a [§REQ-backwards-compatibility.2](../requirements/REQ-backwards-compatibility.md#2-the-deprecation-path), [§REQ-backwards-compatibility.3](../requirements/REQ-backwards-compatibility.md#3-loud-mechanical-migrations), or [§REQ-backwards-compatibility.5](../requirements/REQ-backwards-compatibility.md#5-correcting-a-verdict-another-requirement-forbids) record. Measuring that gap requires a release-to-candidate differential corpus gate in test and release tooling.
