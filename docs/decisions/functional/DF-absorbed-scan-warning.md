...
`workspace_member_root` refuses a member that resolves to its own block root, or outside it, as a hard error. The two look alike and are not, on the test [§DF-absorbed-scan-warning.3](DF-absorbed-scan-warning.md#3-alternatives-considered) sets. That entry has *no* meaning to preserve: no lexical ancestor lists such a root, so nothing gives it a stable alias path, and it is broken at every scope rather than merely unproductive. A configuration that reaches it never worked. A block whose members happen to cover its scan roots, by contrast, is a configuration that loads, resolves, and answers every query it is asked — it just answers about less than its author thinks. Refusing it is a verdict flip on a working repository; refusing the other is refusing to pretend.
...
**An error, as the issue's first option.** Rejected on [§DF-absorbed-scan-warning.2.1](DF-absorbed-scan-warning.md#21-a-warning-because-the-repair-is-a-judgement-rather-than-a-command): it flips the verdict of repositories that load today, with no command to clear it, and it does so at load so no command survives it.
