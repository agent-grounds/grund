...
fn workspace(
    ...
    write(
        &root.join("packages/sub/docs").join(member_file),
        member_doc,
    );
    root
}
    ...
    output
        ...
        .map(|entry| {
            let citations = entry
                .citations
                .iter()
                .map(|citation| format!("{}@{}:{}", citation.id, citation.line, citation.column))
                .collect::<Vec<_>>()
                .join(",");
            format!(
                "{}|{}|{}",
...
fn two_projects_scan_errors_are_one_list_in_path_order() {
    ...
    symlink("FS-gone-target.md", &root.join("alpha/sub/docs/FS-gone.md"));
    assert_eq!(
        cover_at(&root)
            .scan_errors
            .iter()
        ...
        .map(|entry| {
            (
                ...
                entry
                    .citations
                    .iter()
                    .map(|citation| (citation.line, citation.column, citation.text.clone()))
                    .collect::<Vec<_>>(),
            )
        })
        .collect::<Vec<_>>();
