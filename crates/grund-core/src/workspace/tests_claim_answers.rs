...
fn an_ancestor_with_overlapping_members_that_claims_nothing_is_climbed_past() {
    let root =
        test_root("an_ancestor_with_overlapping_members_that_claims_nothing_is_climbed_past");
    for (dir, body) in [
        (
            "outer",
    ...
    assert!(
        format!("{err:#}")
            .contains("workspace members overlap: `packages` contains `packages/api`"),
...
fn an_enclosing_workspace_whose_config_does_not_load_fails_the_narrowed_run() {
    let root =
        test_root("an_enclosing_workspace_whose_config_does_not_load_fails_the_narrowed_run");
    for (dir, body) in [
        (
            "",
            "project_name = \"root\"\n\n[workspace]\nmembers = [\"group\", \"/abs\"]\n",
...
fn an_ancestor_whose_members_text_cannot_be_read_warns_and_lets_the_run_through() {
    let root =
        test_root("an_ancestor_whose_members_text_cannot_be_read_warns_and_lets_the_run_through");
    for (dir, body) in [
        (
            "outer",
            "project_name = \"outer\"\n\n[workspace]\nmembers = [\"repo\"\n",
