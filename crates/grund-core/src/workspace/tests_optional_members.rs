...
fn an_absent_optional_member_does_not_fail_expansion() {
    ...
    assert_eq!(
        aliases,
        vec!["acme".to_string()],
        "only the root is a project here"
    );
}
...
fn include_root_false_with_only_absent_optional_members_is_not_an_empty_block() {
    ...
    let aliases =
        expand(&root).expect("a block whose only members may be absent is not an empty block");

    assert!(
        aliases.is_empty(),
        "no project is in scope, and that is not an error"
    );
}
