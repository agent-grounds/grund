...
Several things in the generated entrypoint are *substituted in* rather than fixed for its `vN`, so the file describes the repo it is in. The generated project name is the explicit `--name`, otherwise the `project_name` from a config at either supported location in the target, otherwise the resolved target basename. That name is interpolated into the scaffolding H1 emitted above the block whenever canonical `AGENTS.md` is created or forcibly regenerated ([§FS-init.2.3.10](FS-init.md#2310-writing-the-block)); an ordinary update still preserves the unmanaged H1. The block's **effective ID grammar and artifact map** come from the `grund.toml` `init` leaves governing the target (an existing config in the target, or the defaults `init` is about to write). Neither the generated name nor the effective configuration is inherited from an ancestor when the target has no config.
...
A single `project_name = "<name>"` key appears at the top above the section tables. `init` consumes it as the generated project-name default when no explicit `--name` is supplied ([§FS-init.2.3.8](FS-init.md#238-substituted-content)); no other non-workspace subcommand consumes it. It also remains metadata that downstream tooling (IDE status bars, CI dashboards) can read without re-deriving the name. Directly below it the file teaches the optional `project_description` key ([§FS-config.3](FS-config.md#3-schema)): a commented `# project_description = "<one line shown next to this project in workspace member lists>"` line by default, or the real `project_description = "<text>"` key when `--description` was given ([§FS-init.1](FS-init.md#1-inputs)). The commented form is a teaching comment like the finite-value-set comments ([§FS-init.2.4.5](FS-init.md#245-finite-value-sets-are-named-in-comments)), not a schema key, so the "no extra keys, no missing keys" guarantee ([§FS-init.2.4.3](FS-init.md#243-every-written-key-is-the-default)) is unchanged.
