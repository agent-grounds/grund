...
- The `[reference] cross_project_when_standalone` opt-in ([§DF-subproject-namespaces.3.6](DF-subproject-namespaces.md#36-standalone-members-fail-loud-not-silent)) remains follow-up work; today the only configurable axis is "error". Any future relaxation must explicitly remain between *error* and *warning*.
