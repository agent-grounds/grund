...
The write half carries more weight, because `fmt` is the one command whose mistakes are structurally invisible afterwards. A wrong expansion is a well-formed citation of a real declaration, so no later pass can see it — [issue #81](https://github.com/agent-grounds/grund/issues/81) was found by a human re-reading a 5,750-line diff. That is [§DF-shorthand-numeric-run.2.7](DF-shorthand-numeric-run.md#27-invention-is-reported-in-full-whatever-the-rule-decides) and [§REQ-no-wrong-citation.3](../../requirements/REQ-no-wrong-citation.md#3-no-wrong-write) stated as a defect class.

**Writing the principle in the spec has already been tried, twice, and failed twice.** The exit-`2` contract was in [§FS-fmt.3](../../functional-spec/FS-fmt.md#3-outputs) *before* the silent-`fmt` defect, and a code comment shows its author knew of the contract and chose plumbing convenience over it. [Issue #105](https://github.com/agent-grounds/grund/issues/105) violates "an unreadable path is fatal up front" — a sentence added to [§FS-fmt.3](../../functional-spec/FS-fmt.md#3-outputs) by the very pull request whose code violated it. A principle enforced per code path by hand does not survive the next code path, and this one did not survive its own change.
...
**Historical note (2026-09-20).** The flat `include!` layout above, and the one-file/one-`include!` consequence recorded in [§DF-fmt-one-model.3](DF-fmt-one-model.md#3-consequences), describe the layout when this decision landed. PR #239 replaced it with one Rust module per component of [§AR-system.2](../../architecture/README.md#2-components), including the writers component in [§AR-system.2.8](../../architecture/README.md#28-writers); the completeness code now lives at `crates/grund-core/src/writers/fmt_complete_findings.rs`. Its nested `mod complete_findings` remains intentional under that component-module shape: keeping the proof field private even from sibling items in the writers component makes the checked constructors the only route to the proof, preserving [§DF-fmt-one-model.2.5](DF-fmt-one-model.md#25-completeness-is-a-type-the-consumer-demands-because-prose-has-failed-twice)'s rationale. [`scripts/check_fmt_complete_findings.sh`](../../../scripts/check_fmt_complete_findings.sh) remains the compile-failure proof for [§FS-fmt.7.4](../../functional-spec/FS-fmt.md#74-no-write-without-a-complete-model), as measured by [§FS-fmt.7.5](../../functional-spec/FS-fmt.md#75-measurable).
...
- **The cost this does not pay off.** #83 is left standing ([§DF-fmt-one-model.2.4](DF-fmt-one-model.md#24-the-rule-binds-fmt-to-checks-model-not-checks-findings-to-fmts-capability)). A shared model is slower to change than a private one, and a `fmt` behavior that needs a walk `check` does not do now has to argue for it in the spec first rather than growing a private pass. That is the intended friction; it is still friction, and the twelve defects above are what it is priced against.
- Rule 7.4 is enforced by the compiler and by a script nobody's gate runs ([§DF-fmt-one-model.2.6](DF-fmt-one-model.md#26-the-compile-time-rule-is-checked-by-compiling-the-wrong-program)). Its weakest link is a reviewer who does not run the script; the alternatives were weaker or dishonest.

## 4. Alternatives considered

| Option | Why rejected |
|---|---|
| State the principle in [§FS-fmt.3](../../functional-spec/FS-fmt.md#3-outputs) and stop | Tried twice, failed twice — once against a contract the violating author had read, once in the same pull request that wrote the sentence ([§DF-fmt-one-model.1](DF-fmt-one-model.md#1-context)). |
...
| Byte equality for reader-equivalence, as the issue words it | Contradicts [§FS-fmt.3](../../functional-spec/FS-fmt.md#3-outputs)'s deliberate `nothing was rewritten: ` prefix on the day it lands ([§DF-fmt-one-model.2.3](DF-fmt-one-model.md#23-reader-equivalence-is-set-equality-not-byte-equality)). |
...
| `trybuild` for the compile-fail assertion | A new dev-dependency whose `.stderr` goldens track compiler versions, against a CI matrix of several operating systems — a recurring maintenance cost for an assertion about the *external* API, which is not where #105 lived ([§DF-fmt-one-model.2.6](DF-fmt-one-model.md#26-the-compile-time-rule-is-checked-by-compiling-the-wrong-program)). |
| A `compile_fail` doctest plus a `cargo test --doc` gate step | Same wrong surface, and it needs a new step in two gate files to run at all. A gate step that proves an adjacent property is read as coverage of the real one ([§DF-fmt-one-model.2.6](DF-fmt-one-model.md#26-the-compile-time-rule-is-checked-by-compiling-the-wrong-program)). |
| Put the completeness check in the producer and audit callers in review | Every defect in [§DF-fmt-one-model.1](DF-fmt-one-model.md#1-context)'s table passed review. |
