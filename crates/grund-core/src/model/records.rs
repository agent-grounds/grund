...
// The one thing `model` reads of grammar: the `Config` record carries the
// compiled `Grammar`, and the two sit at the same level (§AR-system.4).
use crate::grammar::Grammar;
// Upward reads §AR-system.4 forbids once their owners are modules, kept working
// through the crate root until then; the `DEFAULT_*` scan tables are config's.
use crate::{
    AbsentOptionalNamespace, DEFAULT_COMMENT_PREFIXES, DEFAULT_INCLUDE, DEFAULT_KINDS,
    DEFAULT_SCAN_EXTENSIONS, FileStructure, KindConfig, LeadSizeWarning, default_kind_citable,
    default_kind_file, default_kind_folder, default_kind_index, default_kind_title, render_id,
};
