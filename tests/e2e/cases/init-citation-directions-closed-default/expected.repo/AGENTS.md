...
`must`/`never` are `grund check` errors; `should`/`avoid` are suggestions (`grund check --suggestions`). Every source file must cite a declared ID or declare one inline; every file under skills/ must cite one.

- Each **FS** declaration may cite only AR.
- Each **AR** declaration must cite FS; may cite anything else.
- Each file in **skills/** must not cite AR.
Any citation not listed above is forbidden.
