...
use super::index::{INDEX_RULE_PRIOR_RELEASE, INDEX_RULE_RELEASE};
use crate::api::{FmtOpts, format_references, run_check};
use crate::config::display_path;
use crate::testing::{check_run, codes, findings, kind_index_repo, only, test_root, write};
...
fn a_declaration_the_index_does_not_name_is_an_error_at_the_declaration() {
    let root =
        kind_index_repo("a_declaration_the_index_does_not_name_is_an_error_at_the_declaration");
    write(
...
fn a_bare_entry_is_an_error_at_its_line_in_the_index() {
    ...
    assert_eq!(
        finding.line,
        Some(3),
        "the line `grund fmt --write` rewrites"
    );
    assert!(
    ...
    assert!(
        finding
            .message
            .contains(&format!("unchecked in grund {INDEX_RULE_PRIOR_RELEASE}"))
            && finding
                .message
                .contains(&format!("an error in {INDEX_RULE_RELEASE}")),
...
fn index_rule_releases_are_ordered_and_behind_us() {
    ...
    assert!(
        prior < arrival,
        "{INDEX_RULE_PRIOR_RELEASE} < {INDEX_RULE_RELEASE}"
    );
    assert!(
        prior <= current,
