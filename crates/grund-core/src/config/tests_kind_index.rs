...
//! the checker's `tests_kind_index.rs`; this file is about the key alone, so a
//! reader chasing a rejected `grund.toml` is not paging through
//! §FS-check.3.18's cases.
...
fn an_index_outside_its_folder_is_a_config_error() {
    for (case, value) in [
        ...
        let root = test_root(&format!(
            "an_index_outside_its_folder_is_a_config_error_{case}"
        ));
        write(
            &root.join("grund.toml"),
            &format!(
