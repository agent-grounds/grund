...
- No **field** is added to a single-project tree's JSON: `project` appears only in workspace mode (§2.3), so a consumer's parser sees the schema it was written against.
