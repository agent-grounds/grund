...
A site is a whole comment block, and blocks in every doc-comment convention mix kinds of line: a Rustdoc or Javadoc block opens with a summary sentence and carries its `§<ID>: …` lines below it. Judging the block as a unit would either reject that shape or accept a block whose first citation line is malformed because a later one is fine. So a line that carries a recognized citation is judged on its own, lines without a citation are unconstrained, and the finding is anchored at the offending line — which is also the line the author has to edit ([§FS-inline-citation-style.4.4](../../functional-spec/FS-inline-citation-style.md#44-warnings-and-errors--opt-in-layout-deviations)).

Per line is not the same as *every* line, and two shapes drew the boundary. A **wrapped note** may name a second spec point after its opener: `inline_note_max_lines` says a note may run to three lines and [§FS-inline-citation-style.3.3](../../functional-spec/FS-inline-citation-style.md#33-inline_note_layout--where-the-citations-sit) rule 3 says a note may name a second point in passing, but judging every citation-bearing line made those two promises mutually exclusive — the moment the note wrapped, its `(see also §<other>)` half became illegal and the author had to hoist the citation into the opening run, changing what the sentence says. The layout axis is meant to be orthogonal to the size axis, and that was the axis quietly constraining the other. A **list item** was worse: `/// - §<ID>: note` had no conforming spelling at all, because the bullet is content and the run has to open the content, so the enumerated-grounded-points block that Rustdoc, Javadoc, and JSDoc all render as a list was unwritable under the key.

Both are fixed by narrowing *which* lines the rule speaks about rather than by loosening the form ([§DF-inline-note-layout.2.3](DF-inline-note-layout.md#23-one-exact-canonical-form)). A leading list marker is skipped with the indentation, and a citation-bearing line is judged only when it is the site's first one or when it opens with a citation token — a line that opens with prose and names an ID later is the continuation of a note that already opened correctly. What the narrowing costs is stated rather than hidden: a citation written prose-first on a continuation line is not reported, so the rule is a check on how notes *open*, not a guarantee that every `§<ID>` in a block sits in front of a colon. Judging those lines instead would need to know where a sentence ends, which is the judgment [§FS-non-goals.3](../../functional-spec/FS-non-goals.md#3-code-ast-parsing) refuses to acquire — the same reason `grund fmt` stays out of this rule entirely ([§DF-inline-note-layout.2.5](DF-inline-note-layout.md#25-check-only-grund-fmt-is-not-involved)). The first citation-bearing line stays judged unconditionally, because that is the line that opens the note and the one form worth demanding is how a note opens.
...
Exact is about the form, never about the window it is read in. Stripping a comment prefix, the indentation behind it, and a leading list marker ([§DF-inline-note-layout.2.2](DF-inline-note-layout.md#22-per-line-not-per-site)) decides *where the content starts*; requiring `, ` between two citations and a space after the colon decides *what the content must say*. Relaxing the first is how the rule stays a statement about layout rather than about comment formatting; relaxing the second would hand the project back the variation it configured the key to remove.
...
- A repository on `citation-first-colon` gains one more reason for its comments to be mechanically checkable: the citation-first line is the only inline form that cannot be read as an inline declaration, which is what the ambiguity in [§DF-inline-note-layout.1](DF-inline-note-layout.md#1-context) costs today under `strict = false`.
...
| Judge every citation-bearing line, continuations included | Makes a wrapped note and a second cited point mutually exclusive, so `inline_note_max_lines` and rule 3 contradict each other as soon as both are used; the author's only fix is to reword the sentence. [§DF-inline-note-layout.2.2](DF-inline-note-layout.md#22-per-line-not-per-site). |
...
| Normalize in `grund fmt --write` | The rewrite needs sentence-level judgment (where the prose ends, whether the trailing parenthetical was the citation) that no line-oriented formatter has; a mangled comment is worse than a reported one. [§DF-inline-note-layout.2.5](DF-inline-note-layout.md#25-check-only-grund-fmt-is-not-involved). |
