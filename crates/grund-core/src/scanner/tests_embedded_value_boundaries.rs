...
fn reversed_and_duplicate_root_claims_invalidate_catalog_authority() {
    ...
    assert!(
        ["1", "1.1"]
            .into_iter()
            .all(|path| !declaration.sections[path]
                .value_root
                .as_ref()
                .unwrap()
                .valid),
    ...
    assert!(
        report
            .errors
            .iter()
            .any(|error| { error.code == "invalid-value-declaration" && error.line == Some(3) })
    );
    assert!(
        report
            .errors
            .iter()
            .all(|error| error.code != "value-mismatch")
    );
    write(
    ...
    assert!(
        report
            .errors
            .iter()
            .any(|error| { error.code == "invalid-value-declaration" && error.line == Some(5) })
    );
    assert!(
        report
            .errors
            .iter()
            .any(|error| error.code == "duplicate-section")
    );
    assert!(
        report
            .errors
