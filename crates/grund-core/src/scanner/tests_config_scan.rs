...
fn default_config_requires_marker_prefixed_citations() {
    let config = Config::default_for(test_root(
        "default_config_requires_marker_prefixed_citations",
    ));

    assert!(
        config.strict,
        "§FS-config.3.1: strict reference mode is the default"
    );
}
...
fn qualified_citation_suppression_is_uniform_across_passes() {
    ...
    write(
        &root.join("docs/code.md"),
        "See `§api/FS-001-login` here.\n",
    );
