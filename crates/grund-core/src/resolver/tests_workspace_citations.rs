...
fn workspace_boundary_nested_scan_root_is_not_scanned_as_parent_content() {
    ...
    write(&root.join("docs/functional-spec/FS-root.md"), &root_doc);
    write(
        &root.join("apps/api/docs/functional-spec/FS-child.md"),
        "# FS-child: Child\n",
    );
...
fn workspace_qualified_citation_uses_target_id_grammar() {
    ...
    let api_report = check_with_workspace(
        &api_findings,
        &api_config,
        &root_config,
        Some("api"),
...
fn member_local_qualified_citation_with_foreign_grammar_reports_unknown_alias() {
    let root =
        test_root("member_local_qualified_citation_with_foreign_grammar_reports_unknown_alias");
    ...
    assert!(
        findings
            .citations
            .iter()
            .any(|cite| cite.namespace.as_deref() == Some("root") && cite.text == "§root/FS-root"),
...
fn workspace_qualified_dangling_diagnostic_uses_target_id_grammar() {
    ...
    std::fs::create_dir_all(root.join("apps/api/docs/functional-spec")).expect("create api docs");
    ...
    assert!(
        report.errors.iter().any(|error| {
            error.code == "dangling" && error.message == "unknown reference api/FS-001-missing"
        }),
...
"#,
    ...
    let (alias, raw_id) = split_qualified_id_arg("api/FS-001-session").expect("split qualified ID");
    let project = context
