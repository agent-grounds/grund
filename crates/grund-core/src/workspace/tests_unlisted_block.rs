...
fn unlisted_block_repo_at(root: PathBuf) -> PathBuf {
    ...
    write(
        &root.join("a/docs/FS-001-alpha.md"),
    ...
    write(
        &root.join("b/c/docs/FS-001-gamma.md"),
...
fn an_unlisted_workspace_block_is_reported_at_its_own_workspace_line() {
    let root =
        unlisted_block_repo("an_unlisted_workspace_block_is_reported_at_its_own_workspace_line");
...
fn an_unlisted_block_configured_under_agents_is_found() {
    ...
    std::fs::rename(
        root.join("b/.agents-tmp"),
        root.join("b/.agents/grund.toml"),
    )
...
fn include_root_false_on_the_unlisted_block_reports_the_same_finding() {
    let root =
        unlisted_block_repo("include_root_false_on_the_unlisted_block_reports_the_same_finding");
    write(
        &root.join("b/grund.toml"),
        "grund_config_version = 1\nproject_name = \"b\"\n\n\
             [id]\nformat = \"{kind}-{slug}\"\n\n\
...
fn only_the_outermost_unlisted_block_of_a_chain_is_reported() {
    ...
    write(
        &root.join("b/c/d/docs/FS-001-delta.md"),
...
fn plain_repo_absorbing_the_block(name: &str, project_name: Option<&str>) -> PathBuf {
    ...
    let named =
        project_name.map_or_else(String::new, |name| format!("project_name = \"{name}\"\n"));
    write(
        &root.join("grund.toml"),
        &format!(
            "grund_config_version = 1\n{named}\n\
...
/// §FS-check.3.18 records. Read out of the message rather than off the
