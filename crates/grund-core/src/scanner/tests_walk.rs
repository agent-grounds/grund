...
//! with a link it *cannot* read is `tests_walk_errors.rs`. Unix-only: the cases
//! are about symlinks, and creating one on Windows needs developer mode.
...
fn a_config_root_reached_through_a_link_is_reported_under_that_name() {
    let real =
        linked_repo("a_config_root_reached_through_a_link_is_reported_under_that_name-target");
    let link =
        test_root("a_config_root_reached_through_a_link_is_reported_under_that_name").join("repo");
...
fn a_link_to_content_no_project_owns_is_pruned_outside_the_physical_root() {
    let root = test_root("a_link_to_content_no_project_owns_is_pruned_outside_the_physical_root");
    write(
        &root.join("packages/other/docs/functional-spec/FS-001-alpha.md"),
        "# FS-001-alpha: Alpha\n",
    );
...
fn an_outward_directory_link_used_as_a_scan_root_is_pruned() {
    ...
    write(&base.join("shared/FS-002-beta.md"), "# FS-002-beta: Beta\n");
