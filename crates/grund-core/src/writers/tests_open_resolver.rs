...
fn resolver_does_not_evaluate_repository_paths_as_shell_source() {
    ...
    assert!(
        !pwned.exists(),
        "repository path was evaluated as shell source"
    );
    assert!(
...
fn resolver_strips_any_marker_and_keeps_the_workspace_alias() {
    ...
    assert_eq!(
        argv, "app/FS-target.2",
        "alias must survive marker stripping"
    );
...
fn location_matcher_precedes_citation_matcher() {
    ...
    assert!(
        location < citation,
        "wezterm must register the location rule first"
    );
    assert!(
        WEZTERM_SNIPPET.contains("patterns = { grund_location_pattern, grund_citation_pattern }"),
...
fn resolver_ignores_field_shapes_inside_the_body() {
    ...
    assert_eq!(
        opened, "docs/real.md:8",
        "body prose must not shadow the real fields"
    );
}
