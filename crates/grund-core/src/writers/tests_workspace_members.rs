...
use crate::templates::render_grund_toml;
...
fn workspace_members_empty_when_no_workspace_declared() {
    ...
    assert_eq!(
        render_workspace_members_section(&root, None, None, "§", true),
        ""
    );
...
fn workspace_members_self_identity_is_canonical_not_pending_alias_text() {
    ...
    assert!(
        ...
        "pending self alias leaked into: {section}"
    );
    assert!(
        !section.contains("`api`"),
        "self basename leaked into: {section}"
    );
    assert!(section.contains("- [`root`](../../) *(not yet initialized)*"));
}
...
fn workspace_members_self_identity_follows_target_symlink() {
    ...
    std::fs::create_dir_all(root.join("apps/api")).expect("create api");
    std::os::unix::fs::symlink("apps/api", root.join("api-link")).expect("symlink api target");

    let section = render_workspace_members_section(&root.join("api-link"), None, None, "§", true);

    assert!(
        !section.contains("`api`"),
        "canonical self row leaked into: {section}"
    );
    assert!(section.contains("- [`root`](../../) *(not yet initialized)*"));
}
...
fn workspace_members_omits_root_when_include_root_false() {
    ...
    let section = render_workspace_members_section(&root.join("apps/api"), None, None, "§", true);
...
fn workspace_members_renders_configured_descriptions() {
    ...
    assert!(section.contains("- [`api`](apps/api/AGENTS.md): Payment API service"));
    // Uninitialized member: description before the trailing marker.
    assert!(
        section.contains("- [`core`](packages/core/): Core domain library *(not yet initialized)*")
    );
    // Self and its description are omitted together.
    assert!(
        !section.contains("`root`"),
        "self description row leaked into: {section}"
    );
...
fn workspace_members_member_init_omits_pending_self_description() {
    ...
    assert!(
        !section.contains("Billing service"),
        "pending self description leaked into: {section}"
    );
    assert!(section.contains("- [`root`](../../) *(not yet initialized)*"));
}
...
fn workspace_members_nested_grouping_node_without_include_root_has_no_row() {
    ...
    assert!(
        !section.contains("`root`"),
        "root self row leaked into: {section}"
    );
    assert!(
        !section.contains("`group`]"),
        "grouping node must not be a row: {section}"
    );
}
...
fn workspace_members_ignores_an_ancestor_workspace_that_does_not_claim_the_target() {
    let root =
        test_root("workspace_members_ignores_an_ancestor_workspace_that_does_not_claim_the_target");
    write(
        &root.join("grund.toml"),
        "project_name = \"outer\"\n\n[workspace]\nmembers = [\"unrelated\"]\n",
    );
    ...
    assert!(
        !section.contains("`root`"),
        "repository self row leaked into: {section}"
    );
    assert!(
...
fn workspace_members_at_a_group_its_parent_does_not_list_names_its_own_tree() {
    let root =
        test_root("workspace_members_at_a_group_its_parent_does_not_list_names_its_own_tree");
    write(
        &root.join("grund.toml"),
        "project_name = \"root\"\n\n[workspace]\nmembers = [\"listed\"]\n",
    );
    ...
    assert!(
        !section.contains("`stray`"),
        "workspace self row leaked into: {section}"
    );
    assert!(
...
fn grund_toml_description_teaching_line_and_substitution() {
    let teaching_line = "# project_description = \"<one line shown next to this project in workspace member lists>\"";
