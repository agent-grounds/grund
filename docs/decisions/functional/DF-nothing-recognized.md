...
Report every `# <KIND>-…: <title>` heading whose ID fails `[id] format`, at its own `path:line`. This is the near miss [§FS-check.5](../../functional-spec/FS-check.md#5-what-grund-does-not-check) already records as unflagged, and it shipped as [§FS-declarations.checks.declaration-near-miss](../../functional-spec/FS-declarations.md#checksdeclaration-near-miss-declaration-near-miss).
