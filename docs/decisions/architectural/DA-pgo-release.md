...
| **A dedicated PGO training corpus, separate from the self-repo benchmark workload.** Could be tuned independently for the optimizer. | A second source of truth for "the everyday operations that matter" that can drift from [§AR-benchmarks](../../architecture/AR-benchmarks.md#ar-benchmarks-instruction-counting-benchmarks-for-the-hot-cli-commands). Reusing one workload keeps the self-repo regression meter and the optimizer input in lockstep ([§DA-pgo-release.3.1](DA-pgo-release.md#31-the-benchmark-workload-is-already-the-canonical-hot-path)). |
