...
The boundary, the one file outside it, where a test module sits, and the two tests that hold this against the tree are [§AR-core-module-layout.1.1](AR-core-module-layout.md#11-modrs-is-the-components-whole-boundary) to [§AR-core-module-layout.1.4](AR-core-module-layout.md#14-two-tests-hold-the-layout-against-the-tree); what a split of an oversized owner owes is [§AR-core-module-layout.1.5](AR-core-module-layout.md#15-split-ownership).
...
As the crate entrypoint it holds the twelve `mod` lines, the explicit `pub use <component>::{…}` list that **is** the crate's public surface ([§AR-core-module-layout.2](AR-core-module-layout.md#2-refactor-boundary)), and `#[cfg(test)] pub(crate) mod testing;` for the fixtures every test module shares. It holds no implementation and re-exports nothing by glob, so a name is public because this list says so rather than because a module happened to leave it `pub`. Every test module sits in its component ([§AR-core-module-layout.1.3](AR-core-module-layout.md#13-a-test-module-sits-beside-the-code-it-pins)), so `lib.rs` carries neither a `#[cfg(test)]` prelude of private globs nor an `include!` line: there is no flat module left for either to serve.
