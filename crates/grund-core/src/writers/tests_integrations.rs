...
#[cfg(unix)]
use super::init_notes::shadowed_claude_entrypoint_note;
...
fn integrations_block_appends_then_is_idempotent() {
    let (appended, outcome) =
        install_managed_block("#", false, "# my config\n", "SNIPPET").unwrap();
    ...
    assert_eq!(
        ...
        "re-applying the same snippet is a no-op"
    );
...
fn integrations_block_upgrades_older_version_in_place() {
    ...
    let (updated, outcome) =
        install_managed_block("#", false, old, "NEW").expect("upgrade old block");
...
fn resolver_peek_prints_the_declaration_without_opening_an_editor() {
    ...
    assert!(
        stdout.contains("RENDERED BODY"),
        "peek must show the declaration"
    );
    assert!(
        !opened.exists(),
...
fn wezterm_wiring_note_reads_only_outside_the_block() {
    let block_only = install_managed_block("--", true, "", WEZTERM_SNIPPET).expect("install block");
    assert!(
        block_only
            .0
            .contains("function grund_apply_hyperlink_rule(config)"),
...
fn conversation_target_is_recorded_independently() {
    ...
    assert!(
        scan.problems[0]
            .1
            .contains("must be one of file | path | web"),
        "{:?}",
        scan.problems
    );
