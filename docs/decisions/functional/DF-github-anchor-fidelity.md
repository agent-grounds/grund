...
- [§GOAL-no-silent-breakage](../../goals.md#goal-no-silent-breakage-changes-ship-through-a-deprecation-path) — the point of the change: the emitted anchor now resolves where the feature says it does.
...
- `anchor_slug_github` in `crates/grund-core/src/grammar/anchors.rs` is rewritten to the algorithm above; its doc-comment cites this record.
