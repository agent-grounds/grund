...
use super::*;
use crate::checker::check_agent_block_path;
...
fn issue_49_deprecated_adapter_accepts_check_finding_selectors() {
    ...
    assert_eq!(
        command_check(&[path.clone(), "--only=--only=agents-init".to_string(),]),
        ExitCode::from(2),
    );
    assert_eq!(
        command_check(&[path.clone(), "--ignore=--ignore=agents-init".to_string(),]),
        ExitCode::from(2),
    );
    assert_eq!(
        command_check(&[
