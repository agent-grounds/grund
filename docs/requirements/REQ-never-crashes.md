...
An input the scanner cannot handle yields an error naming the path ([§FS-errors.2.2](../functional-spec/FS-errors.md#22-cli-level-message)), and a run that could not complete exits `2` ([§FS-check.2](../functional-spec/FS-check.md#2-outputs)) — the failure is reported in-band, never thrown as a stack trace.
