...
[§FS-check.3.9](../../functional-spec/FS-check.md#39-section-heading-level-mismatch) reads the recorded map, so under [§DF-duplicate-section-path.2.3](DF-duplicate-section-path.md#23-the-map-keeps-the-first-heading-and-that-is-now-written-down) it now judges the first heading where it used to judge the last. The duplicates are not additionally level-checked.
...
No tree loses a *finding* to this, though it does not keep the same one: where [§FS-check.3.9](../../functional-spec/FS-check.md#39-section-heading-level-mismatch) used to emit a located error at the second heading's line, [§FS-check.3.16](../../functional-spec/FS-check.md#316-duplicate-section-path) now emits one located error at the first, carrying every colliding line — the second's included — in its `sites` list and in its message. The run still fails, still on the same pair of lines, and the message still names the line [§FS-check.3.9](../../functional-spec/FS-check.md#39-section-heading-level-mismatch) used to anchor at; what moves is which line the finding is anchored to, and it moves to the one the section resolves to.
...
The rule this project actually holds is that ambiguity is reported rather than ranked, and [§DF-duplicate-section-path.2.1](DF-duplicate-section-path.md#21-the-collision-is-an-error-in-fs-check33s-shape) keeps it at both levels.
