...
/// vacuously.
fn home() -> PathBuf {
    std::env::home_dir().expect("these cases assert against the real home directory; it is unset")
}
...
fn user_global_instruction_files_are_refused() {
    ...
    for global in [
        ...
        let message =
            refusal.unwrap_or_else(|| panic!("{global} is a user-global file and was allowed"));
        assert!(
