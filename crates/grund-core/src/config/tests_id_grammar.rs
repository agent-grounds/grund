...
fn id_grammar_rejects_a_slash_in_every_component() {
    ...
    for (body, expected, line) in cases {
        ...
        assert!(err.contains(expected), "unexpected error for {body}: {err}");
        assert!(
...
fn id_grammar_rejects_a_pattern_that_matches_a_slash_without_containing_one() {
    let root =
        test_root("id_grammar_rejects_a_pattern_that_matches_a_slash_without_containing_one");
    for (pattern, key) in [
        ("[^.[:space:]]+", "slug_pattern"),
        (".+", "slug_pattern"),
        ("[!-9]+", "slug_pattern"),
...
fn id_grammar_accepts_a_pattern_that_forbids_a_slash() {
    ...
    for pattern in ["[^/. ]+", "[^/.]+", "(?:a|b)[^/.]*", "[a-z][a-z0-9-]*"] {
        ...
        let config = load_config(&root).unwrap_or_else(|err| {
            panic!("`{pattern}` cannot produce a `/` and must load: {err:#}")
        });
