...
use crate::api::{CheckOpts, CheckRun, check_with_opts, lsp_snapshot};
use crate::model::{Diagnostic, Finding};
use crate::queries::LspSnapshotOpts;
use crate::testing::{check_run, findings, located_diagnostics, test_root, write};
...
fn a_tree_with_no_near_miss_still_earns_the_caution() {
    ...
    assert_eq!(
        caution.path, None,
        "§FS-check.2.1.1: a CLI-level message names no site"
    );
    ...
    assert!(
        caution
            .message
            .contains("[id] format = \"{kind}-{number}-{slug}\""),
...
fn a_walk_that_read_no_files_keeps_the_empty_scan_caution() {
    ...
    assert!(
        caution(&run).is_none(),
        "no file was read, so nothing was there to recognize"
    );
    assert!(
        run.report
            .warnings
