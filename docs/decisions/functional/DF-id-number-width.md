...
- **Default width 4.** Rejected. It does not *solve* the rollover, it relocates it from 999 to 9999 — a threshold even fewer repos reach. The price is a leading `0` on *every* ID in *every* repo, forever (`FS-0008-user-can-log-in`), plus a mismatch with the documented `-NNN-` form — a universal, unconditional cost paid to defend against an event that is both extremely rare and (per [§DF-id-number-width.4](DF-id-number-width.md#4-what-happens-at-the-rollover)) harmless when it does occur. The asymmetry runs the wrong way: pay the tiny rare cost, not the small universal one.
- **An `[id] number_width` config key (default 3) that `grund id` reads when `--width` is omitted.** Not done now, but the door is left open: it is additive (a config without the key behaves exactly as today), needs no `grund_config_version` bump ([§FS-config.5](../../functional-spec/FS-config.md#5-schema-versioning)), and mirrors how `number_pattern` already lives in `[id]`. Its case is a repo that *knows* it will be large and wants the width pinned without every contributor remembering `--width` — the [§GOAL-small-and-large](../../goals.md#goal-small-and-large-start-small-configure-for-big) / [§GOAL-configurable](../../goals.md#goal-configurable-every-default-is-overridable) instinct. Until such a repo asks for it, a CLI flag plus a documented default is enough; adding the key now is config surface for a need no one has expressed.

## 6. Consequences

- `command_id` in `crates/grund-core/src/compat/id.rs` keeps `let mut width = 3usize;` as the default, overridable by `--width`.
...
- No change to the `[id]` config schema, `templates/grund.toml`, or `grund config show` — the width is deliberately not a config key ([§DF-id-number-width.5](DF-id-number-width.md#5-alternatives-considered)).
