...
A shorthand that matches no declaration, or more than one, is **left alone**: `fmt` normalizes, it does not guess, and `check` is where the ambiguity is reported. Trigger-origin shorthand remains authoring sugar under both policies, so `$$FS-042` becomes `§FS-042-user-login` in one step. On a line that also contains persisted `§FS-042`, `accepted` expands only the trigger; origin is decided per candidate, not per line (§2.1). The never-rewrite rules of §2.3 apply unchanged — and because the ones for inline code, a Markdown link destination, and a source string literal do, [§FS-check.3.13](FS-check.md#313-number-only-shorthand-citation) withholds its canonical-policy error there, so no finding in those places asks for a rewrite this pass refuses to make.
