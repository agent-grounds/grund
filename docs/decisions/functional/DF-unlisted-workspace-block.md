...
The finding is a **warning**, and its message names the release in which it becomes an error: `0.15.0`, recorded now as the release it did become one in ([§FS-check.3.29.14](../../functional-spec/FS-check.md#32914-an-error-because-the-deadline-the-warning-named-has-arrived)).
...
**Consequence — the ramp completed in `0.15.0`.** What this section decided was *which ramp applied*, and that stays true now that it has run its course: `check` reports the finding as an error, the clause in the message text reports the release in the past tense, and the test that held the date ahead of the running version is gone with the date it held ([§FS-check.3.29.14](../../functional-spec/FS-check.md#32914-an-error-because-the-deadline-the-warning-named-has-arrived)). The five walking surfaces that have no error channel keep the warning.
...
A located finding on stdout for `check` plus a CLI-level line elsewhere was considered and rejected. Two shapes for one fact is the divergence this decision exists to prevent, and it would make the text a consumer greps for depend on which command produced it. The cost is paid under `--format=json`, where a CLI-level diagnostic carries `path` and `line` `null` ([§FS-errors.5](../../functional-spec/FS-errors.md#5-json-format)) and the location lives in the message text. That is the same cost [§FS-check.4.3](../../functional-spec/FS-check.md#43-redundant-config-pair) and the deprecated-location warning of [§FS-check.4.11](../../functional-spec/FS-check.md#411-config-read-from-the-deprecated-agents-location) already pay, and it buys one grep across six commands.

**Superseded for `check` and the editor, and only there.** Once the ramp of [§DF-unlisted-workspace-block.2.1](DF-unlisted-workspace-block.md#21-a-warning-that-names-the-release-it-becomes-an-error-in) ended, the finding stopped being a warning on the one surface that has an error channel, and the located shape this section rejected is the shape an error has to wear: [§DF-unlisted-workspace-error-shape](DF-unlisted-workspace-error-shape.md#df-unlisted-workspace-error-shape-check-and-the-editor-take-the-located-shape-the-five-walking-surfaces-keep-the-cli-level-line) decides that, and decides it for `check` and the editor alone. The reasoning above is *why one shape was right for a warning*, and it stays right for the five walking surfaces that still print one — which is why it is superseded in part rather than rewritten.

In `check` the warning is one of the report's warnings rather than a line printed past it. That is load-bearing twice over: it is what makes the warning stand in place of the `success` marker ([§FS-check.2.1](../../functional-spec/FS-check.md#21-report-format)), so the verdict is catchable, and a catchable verdict is what makes [§DF-unlisted-workspace-block.2.1](DF-unlisted-workspace-block.md#21-a-warning-that-names-the-release-it-becomes-an-error-in)'s deprecation-path reasoning correct in the first place — [§REQ-backwards-compatibility.1](../../requirements/REQ-backwards-compatibility.md#1-what-is-covered) governs this as a verdict change precisely because a warning displaces `success`.
...
A repository that has an unlisted block sees one new warning per outermost such block and keeps its exit code, until the release the warning names. Because a warning displaces `success`, a repository gating on that marker rather than on the exit code sees the change immediately — which is the point of a ramp that gives it a release to act in.

**Consequence — that release arrived.** `0.15.0` is the release, and in it `grund check` exits `1` over such a block instead of `0` ([§FS-check.3.29](../../functional-spec/FS-check.md#329-unlisted-workspace-block)). A repository that took the ramp's offer — one of the two one-line config edits — sees nothing change; one that skipped the warning for two releases sees CI go red, which is the deprecation path working rather than failing. The shape the finding takes there, and the JSON surface that moves with it, are [§DF-unlisted-workspace-error-shape](DF-unlisted-workspace-error-shape.md#df-unlisted-workspace-error-shape-check-and-the-editor-take-the-located-shape-the-five-walking-surfaces-keep-the-cli-level-line)'s.
...
| **`check` only, on the [§FS-check.4.3](../../functional-spec/FS-check.md#43-redundant-config-pair) analogy.** One command pays for the claim resolution. | The analogy fails: a redundant pair is a fact about which file the run read, this is a fact about how every command spells its projects — and the ticket's own repro is a `grund list` ([§DF-unlisted-workspace-block.2.3](DF-unlisted-workspace-block.md#23-every-command-that-walks-carries-it-not-check-alone)). |
| **A located finding on stdout for `check`, CLI-level elsewhere.** Gives `check` a `path:line:` an editor can jump to. | Two shapes for one fact, and a message whose text depends on the command that printed it ([§DF-unlisted-workspace-block.2.4](DF-unlisted-workspace-block.md#24-one-shape-on-every-surface)). |
