...
`pre-commit run --all-files` reproduces only the stages whose input is a file list. A hook bound to a stage with different input — `commit-msg`, whose argument is a message file — is silently absent from that run, so "CI runs the config" is not by itself parity. Every such hook needs an explicit CI counterpart that feeds it the input the stage would have supplied; [§AR-ci.8](AR-ci.md#8-commit-message-attribution-gate) is the first.
