...
use super::citation_directions::CITATION_LEVEL_LEGEND;
use super::*;
use crate::config::load_config;
use crate::grammar::markdown_fence_delimiter;
...
"#,
    ...
    assert!(
        section.contains("- Each **FS** declaration should cite FS."),
        "{section}"
    );
    assert!(
        section.contains("- Each file in **skills/** must cite FS."),
        "{section}"
    );
    // A single-file home is one file, so "each file in" would promise a
    // directory that is not there.
    assert!(
        section.contains("- The file **docs/runbook.md** must cite FS."),
        "{section}"
    );
    assert!(
        section.contains(
...
"#,
    ...
    assert!(
        section.contains("must cite (FS or GOAL) and AR."),
        "{section}"
    );
...
"#,
    ...
    assert!(
        section.contains("avoid citing AR in any project."),
        "{section}"
    );
    assert!(
        !section.contains("*/AR"),
        "rule grammar must not reach the prose: {section}"
    );
}
...
"#,
    ...
    assert!(
        section.contains("- Each **AR** declaration may cite only FS or GOAL."),
        "{section}"
    );
    assert!(
        !section.contains("unlisted citations"),
        "one clause, not two: {section}"
    );
}
...
"#,
    ...
    assert!(
        section.contains(
            "- Each **AR** declaration must cite FS; may cite GOAL; never cite anything else."
        ),
        "{section}"
    );
}
...
fn an_open_per_kind_default_speaks_only_under_a_closed_global_one() {
    ...
    assert!(
        open.contains("- Each **FS** declaration must cite AR."),
        "{open}"
    );
}
...
fn the_closing_line_reports_the_global_default() {
    ...
    for global in ["may", "must", "should"] {
        ...
        assert!(
            section
                .trim_end()
                .ends_with("Anything not listed above is allowed."),
            "`{global}` closes as open: {section}"
        );
        assert!(
            !section.contains("By default,"),
            "no top default sentence: {section}"
        );
    }
    ...
    assert!(
        forbidden
            .trim_end()
            .ends_with("Any citation not listed above is forbidden."),
        "{forbidden}"
    );
    let discouraged = render_shared(
    ...
    assert!(
        discouraged
            .trim_end()
            .ends_with("Any citation not listed above is discouraged."),
        "{discouraged}"
    );
}
...
"#,
    ...
    assert!(
        !section.contains("templates/"),
        "an unwalked home is never scanned: {section}"
    );
}
...
fn grounding_shrinks_to_the_source_file_rule_and_disappears_when_off() {
    ...
    assert!(
        off.contains(CITATION_LEVEL_LEGEND),
        "the legend stands alone: {off}"
    );
}
...
fn grounding_renders_without_a_citations_section() {
    ...
    assert!(
        section.contains("Specs cite goals, architecture cites specs"),
        "{section}"
    );
    assert!(
        section.ends_with("Every source file must cite a declared ID or declare one inline."),
        "{section}"
    );
}
...
fn documented_citation_directions_example_matches_production_render() {
    let page_path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../docs/user-facing/citation-directions.md");
