...
This project uses [`grund`](https://github.com/agent-grounds/grund): every spec, goal, decision, and end-to-end test has a stable ID `<KIND>-<slug>[.<section>]` (`KIND ∈ {GRUND, GOAL, FS, AR, DA, RM}`), cited with the marker `§` — e.g. `<§>FS-user-login.3.1` (the `FS-user-login` here is a shape illustration, not a real ID in this repo, hence the `<§>` escape). Type `$$` in a grund-aware editor and it becomes `§`. Bare ID-shaped tokens are ignored — `[reference] strict = true` is set in `grund.toml`, so only `§`-prefixed citations are checked.
...
- [GRUND](docs/grund.md): Declaration
- [GOAL](docs/goals.md): Declaration
- [FS](docs/functional-spec): Declaration
- [AR](docs/architecture): Declaration
- [DA](docs/decisions/architectural): Declaration
- [RM](docs/roadmap.md): Declaration
- [skills/](skills): Agent skills
- [tests/e2e/](tests/e2e): User scenarios
...
- **FS** should cite GOAL or FS; never cite AR; avoid citing GRUND.
- **AR** may cite FS or GOAL; unlisted citations are forbidden.
- **DA** must cite FS or GOAL and AR.
- **skills/** must cite FS or AR or RM; never cite api/AR.
- **tests/e2e/** must cite FS and GOAL; avoid citing */AR.
- **code** (Gradle build and workflows) should cite FS or AR.
Unlisted kinds and pairs follow their configured defaults.
