...
- `--force` — overwrite files that already exist at the target paths (§3). Off by default. Existing entrypoints are appended to or updated without it (§2.3), so it is only needed to reset a generated `AGENTS.md` or a `--docs` scaffold to its canonical bytes.
...
The home directory is whichever one the platform reports, so the rule holds wherever `init` runs. The user-global table §1.2.2 checks is separately `$HOME`-rooted ([§FS-integrations.4](FS-integrations.md#4-managed-writes---write)), so on a platform that does not set that spelling this rule is the one that still answers for `<path>` at the home directory itself.
