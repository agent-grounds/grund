...
fn unknown_and_ambiguous_shorthands_are_named_not_guessed() {
    ...
    assert!(
        !errors
            .iter()
            .any(|message| message.contains("unknown reference")),
        "{errors:?}"
    );
    assert!(
        report
            .errors
            .iter()
            .all(|finding| finding.code == "shorthand-citation")
    );
}
...
fn number_less_and_slug_less_formats_have_no_shorthand() {
    ...
    assert!(
        !messages(&report)
            .iter()
            .any(|message| message.contains("shorthand")),
        "{:?}",
        messages(&report)
    );
...
fn a_resolved_shorthand_grounds_its_source_file() {
    ...
    assert!(
        !messages(&report)
            .iter()
            .any(|message| message.contains("ungrounded")),
        "{:?}",
        messages(&report)
    );
}
