...
fn full_scope_does_not_resolve_an_in_scope_shorthand_against_the_wider_walk() {
    let root =
        test_root("full_scope_does_not_resolve_an_in_scope_shorthand_against_the_wider_walk");
    write(
        &root.join("grund.toml"),
        "grund_config_version = 1\n\n[id]\nformat = \"{kind}-{number}-{slug}\"\n\n[scan]\ninclude = [\"docs\"]\n",
    );
...
fn full_scope_reports_a_qualified_shorthand_once() {
    ...
    write(
        ...
        &member.replace("{alias}", "api"),
    );
    write(
        &root.join("web/grund.toml"),
        &member.replace("{alias}", "web"),
    );
    write(
    ...
    write(
        &root.join("api/internal/AR-900-hidden.md"),
        "# AR-900-hidden: Hidden\n\nLead.\n",
    );
    write(
        &root.join("web/docs/notes.md"),
        "Qualified shorthand: §api/AR-900\n",
    );
