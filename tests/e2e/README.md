...
Name the most-specific leaf the case actually proves. The functional-spec coverage gate treats a parent and child as distinct evidence: a case that asserts `FS-id.2.2` lists that point, not only `FS-id`. The local-only `fetch-*`, `id-*`, value-JSON, query, watch-rejection, and init no-write cases are the reference families for byte-exact streams, exits, and final-tree assertions ([§AR-goal-measurement.1](../../docs/architecture/AR-goal-measurement.md#1-rule)).

A fixture's config is a bare `grund.toml` at the repo root. Both discovery names still work ([§FS-config.1](../../docs/functional-spec/FS-config.md#1-file-location-and-discovery)), but `.agents/grund.toml` is the deprecated one and a run that reads it says so ([§FS-config.1.2](../../docs/functional-spec/FS-config.md#12-the-agents-location-is-deprecated), [§FS-check.4.11](../../docs/functional-spec/FS-check.md#411-config-read-from-the-deprecated-agents-location)) — so a case left on the old path drags that warning into a golden the case is not about, and every case copied from it teaches the form the tool asks a repository to leave. The cases that stay on `.agents/` are the ones whose subject is discovery itself: the `config-agents-deprecated-*` set, `config-redundant-pair-warning`, `config-discovered-from-subdir`, `workspace-bare-member-config`, and `config-unknown-key`, which keeps the config-error path through the old location covered.
...
or opt-in `suggestion:` channel next, and group errors before warnings before
...
- a config discovered at the deprecated `.agents/grund.toml` ([§FS-check.4.11](../../docs/functional-spec/FS-check.md#411-config-read-from-the-deprecated-agents-location), [§FS-config.1.2](../../docs/functional-spec/FS-config.md#12-the-agents-location-is-deprecated), [grund#189](https://github.com/agent-grounds/grund/issues/189)): one line on `check`, on `config validate` and on `config show`, standing in place of `success` and leaving the exit code alone, carried under `--format json` with its `deprecated-config-location` code and null `path`/`line`, and a workspace naming only the member whose config is on the old path — with the five commands that must stay silent beside them (`list`, `refs`, `cover`, `fmt`, and the bare ID read), which are what keep a fact that never changes between runs out of every surface that is not about it
- a `[workspace]` block no enclosing block lists ([§FS-check.4.8](../../docs/functional-spec/FS-check.md#48-unlisted-workspace-block), [grund#72](https://github.com/agent-grounds/grund/issues/72)): the ticket's own tree earning one warning that names the block's `[workspace]` line and stands in place of `success`, `grund list` carrying the same line beside the absorbed spelling it prints, and the two edits that clear it — listing the block as a member, and keeping it out of the enclosing `[scan]` — each silent
- a workspace member the config declares may be legitimately absent ([§FS-workspace.2.2](../../docs/functional-spec/FS-workspace.md#22-a-member-that-may-be-legitimately-absent), [grund#85](https://github.com/agent-grounds/grund/issues/85)): the ticket's own repro with the member present and absent, the absent run naming the unverified namespace on **stdout** and standing in place of `success` while the citation into it goes unreported ([§FS-check.4.9](../../docs/functional-spec/FS-check.md#49-a-workspace-member-declared-optional-is-absent)) — with the four shapes that hold the default and the alias rule in place beside it: a member absent from plain `members` still fatal and now naming the key, a present member whose `project_name` disagrees with its entry's last segment, a trailing `/*` refused with the shape to write instead, and a nested block's own absent optional member reported at its own line
- a `fmt` run narrowed below a workspace root, walking symlinked files out of the root project's scanned tree ([§FS-workspace.6](../../docs/functional-spec/FS-workspace.md#6-nested-project-boundary)): a file owned by a member is the stronger workspace boundary, so `--write` leaves that file and the member's own copy byte-identical, while an external file no project owns remains followed and is rewritten — file links are unchanged by the outward-directory rule
- the issue-102 workspace pair: the root run stays green with `packages/other/docs/blink -> ../../sub`, and the independently checked `other` member also stays green without absorbing `sub` or reporting the shared local ID as a duplicate ([§FS-workspace.6](../../docs/functional-spec/FS-workspace.md#6-nested-project-boundary))
- `include_root = false` leaving the excluded root's own files scanned by nobody: a dangling citation there passes even `check --full`, and the run now says so ([§FS-check.4.10](../../docs/functional-spec/FS-check.md#410-include_root--false-leaves-the-blocks-own-files-unread), [grund#71](https://github.com/agent-grounds/grund/issues/71)) — the ticket's own nested tree under `check`, `check --full`, `list` and `--format json`, the single-level root the ticket calls pre-existing in kind, a run rooted at the opted-out block itself, and two opted-out blocks in one tree earning one line each at their own `include_root` line
- the five configurations that must stay **silent** under that rule, which are what keep it from being noise: a grouping directory that only groups and writes no `[scan] include` at all, a block whose own roots are all inside its members, a tree whose only files are of an unscanned type, a tree whose content is excluded, gitignored, hidden-named or a hidden file, and roots the config names that are not on disk
...
- the unknown-alias hint at the workspace root (the `FS-check.3.8` worked examples, byte-exactly): a grouping path that is a proper segment-prefix of a deeper project in text and JSON while the citation remains unresolved and the JSON code stays `unknown-project`, a dropped prefix naming one project, and a leaf name two projects share naming both, joined as `a or b`
...
- the same candidate clause one level down, on an unqualified `<ID>` a member declares and the current project does not ([§FS-workspace.8.1.1](../../docs/functional-spec/FS-workspace.md#811-an-unqualified-id-another-project-declares), [grund#86](https://github.com/agent-grounds/grund/issues/86)): the ticket's own tree naming `vendored/SPEC-007-shipping` while still refusing, four members declaring one ID listed sorted and cut at three, the same text re-parsed under a member's numbered grammar where the root's is slug-only, an inline section that the candidate does not carry, `--format json` keeping `not-found` with the clause inside `message`, a member-local run reporting bare with the `grund list` hint it has always printed, and the same bare pair in text and JSON from a run that loaded the whole workspace and found the ID nowhere in it
- an off-grammar declaration (`# FS-billing:` under a numbered format) retained
  as readable compatibility ground: `check` reports its located format mismatch
  at exit `0`, while `list` includes it beside the conforming sibling
  ([§FS-config.3.2](../../docs/functional-spec/FS-config.md#32-id--id-grammar),
  [§FS-list.2](../../docs/functional-spec/FS-list.md#2-behaviour))
...
- a directory symlink whose canonical target escapes an independently checked project, with declarations and dangling citations below it absent while the external file-link case beside it remains followed ([§FS-config.3.5.1](../../docs/functional-spec/FS-config.md#351-a-symlink-in-the-tree-is-followed))
- a workspace where a link inside one member reaches a sibling project and another reaches the root project's docs: neither directory crosses, while an in-root file link remains followed
...
- the index a folder kind keeps ([§FS-check.3.18](../../docs/functional-spec/FS-check.md#318-declaration-missing-from-its-kinds-index), [§FS-check.3.17](../../docs/functional-spec/FS-check.md#317-index-entry-is-not-a-link)): a declaration the index does not name, a folder with no index file at all, an entry present but bare, `index = false` opting a kind out, `index = "<name>"` selecting another file, the recursive subtree, a stub-and-inline pair collapsing to one entry — which is also the anchorless link to a source home, accepted because "full link" means the link `fmt` writes here — a canonical source link enrolling the inline declaration directly with no stub, and an inline-code mention counting as neither an entry nor a finding. Every case that reaches the missing-entry rule exits `1`: the ramp [§DF-index-compatibility-ramp](../../docs/decisions/functional/DF-index-compatibility-ramp.md#df-index-compatibility-ramp-a-findings-ramp-follows-its-fix-command-not-the-size-of-the-offence) opened ended in the release this corpus was last updated for, and the message it prints reports that release in the past tense rather than promising it — which is what makes these goldens the bytes the release gate reads the ramp out of ([§FS-distribution.4.2](../../docs/functional-spec/FS-distribution.md#42-a-release-may-not-contradict-the-releases-the-trees-own-messages-name)). `check-index-missing-entry-json` is the same tree under `--format json`, and is what holds the two halves of that verdict apart — the diagnostic's `severity` moved to `error`, and its `code` did **not** move with it, because a code is the stable identifier a consumer greps for ([§FS-errors.5](../../docs/functional-spec/FS-errors.md#5-json-format))
...
- the line-cap and soft-cap findings name the site they measured — the block's line span and the citations that made it one, as written, in source order, deduplicated — and carry the block-splitting rule as a fix-it clause; `inline-note-cap-names-site` pins the shape against a five-line block citing two sections of one ID ([grund#117](https://github.com/agent-grounds/grund/issues/117))
