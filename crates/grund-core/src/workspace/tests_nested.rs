...
//! reason: the shape of the alias path a nested tree produces. The claimed
...
fn workspace_member_resolving_to_its_own_block_is_rejected() {
    ...
    std::os::unix::fs::symlink(&root, root.join("self")).expect("point a member at its own block");
...
fn an_unreadable_glob_parent_is_a_located_members_error() {
    ...
    assert_eq!(
        format!("{err:#}"),
        format!("group/grund.toml:4: cannot read workspace member glob `packages/*`: {read_error}")
    );
}
